...
_SCHEDULER_GUARD = asyncio.Lock()
_ACTIVE_SCHEDULER_TASK: Optional[asyncio.Task] = None
_ACTIVE_SCHEDULER_OWNER: Optional[str] = None
...
class DailyAINewsPlugin(Star):
    ...
    async def initialize(self):
        ...
        await self._sync_command_subscription_fields()
        await self._ensure_single_scheduler()
    ...
    async def _try_fetch_and_push(self, today: str) -> bool:
        ...
        try:
            ...
            async with _PUSH_LOCK:
                # 多实例并发时，从磁盘重新读取一次，避免各实例使用旧的内存快照。
                await self._load_sent_news()

                if article_date in self._sent_dates:
                    logger.info(f"今日 ({article_date}) 已推送过，跳过本次发送")
                    return False
                ...
                return article_date in self._sent_dates or article["link"] in self._sent_links
    ...
    async def _fetch_rss_latest(self) -> Optional[Dict]:
        ...
        try:
            async with aiohttp.ClientSession(
                ...
                article = await self._fetch_latest_from_rss(session, headers)
                if article:
                    return article
                return await self._fetch_latest_from_homepage(session, headers)
        except Exception as e:
            logger.error(f"AI 早报获取失败: {type(e).__name__}: {e!r}")
            return None

    async def _fetch_latest_from_rss(self, session, headers: Dict[str, str]) -> Optional[Dict]:
        """从 RSS 源获取最新文章。"""
        try:
            async with session.get(RSS_URL, headers=headers) as resp:
                if resp.status != 200:
                    logger.warning(f"RSS 请求返回状态码 {resp.status}，准备回退到官网首页")
                    return None
            ...
            if channel is None:
                logger.warning("RSS XML 中未找到 channel 元素，准备回退到官网首页")
                return None

            item = channel.find("item")
            if item is None:
                logger.warning("RSS 中没有任何文章，准备回退到官网首页")
                return None
            ...
            if not title:
                logger.warning("RSS 文章标题为空，准备回退到官网首页")
                return None
            ...
            logger.error(f"RSS XML 解析失败，准备回退到官网首页: {e}")
        except Exception as e:
            logger.error(f"RSS 获取失败，准备回退到官网首页: {type(e).__name__}: {e!r}")
        return None

    async def _fetch_latest_from_homepage(self, session, headers: Dict[str, str]) -> Optional[Dict]:
        """RSS 不可用时，从官网首页解析最新文章。"""
        try:
            async with session.get(HOME_URL, headers=headers) as resp:
                if resp.status != 200:
                    logger.warning(f"官网首页请求返回状态码 {resp.status}")
                    return None
                html_text = await resp.text()

            match = re.search(
                r'<a[^>]+href=["\'](?P<href>[^"\']*issue-\d+/?[^"\']*)["\'][^>]*>\s*(?P<date>\d{4}-\d{2}-\d{2})\s*</a>',
                html_text,
                re.IGNORECASE,
            )
            if not match:
                logger.warning("官网首页未解析到最新 AI 早报文章")
                return None

            article_date = match.group("date")
            link = urljoin(HOME_URL, html.unescape(match.group("href")))
            title = f"AI 早报 {article_date}"
            ...
            async with session.get(link, headers=headers) as resp:
                if resp.status == 200:
                    ...
                    if title_match:
                        parsed_title = self._clean_html(title_match.group("title"))
                        if parsed_title:
                            title = parsed_title
                    content = self._clean_html(article_html)
                else:
                    logger.warning(f"官网文章页请求返回状态码 {resp.status}: {link}")

            logger.info(f"官网首页获取到最新文章：{title}")
            return {
                "title": title,
                "link": link,
                "content": content,
            ...
            logger.error(f"官网首页获取失败: {type(e).__name__}: {e!r}")
            return None
    ...
    def _format_command_subscription_entry(
        self, umo: str
    ) -> tuple[Optional[str], Optional[str]]:
        """将 unified_msg_origin 转成配置面板可读的展示条目。"""
        parts = umo.split(":", 2)
        if len(parts) != 3:
            return None, None

        platform_name, msg_type, target_id = parts
        display = (
            target_id
            if platform_name == "default" and target_id.isdigit()
            else f"{platform_name}:{target_id}"
        )
    ...
    async def _sync_command_subscription_fields(self):
        """将指令订阅状态同步到配置文件，便于配置面板展示。"""
        groups: Set[str] = set()
        users: Set[str] = set()

        for umo in self._cmd_subscriptions:
            group_entry, user_entry = self._format_command_subscription_entry(umo)
            if group_entry:
                groups.add(group_entry)
            if user_entry:
                users.add(user_entry)

        group_text = "\n".join(sorted(groups))
        user_text = "\n".join(sorted(users))

        if (
            self.config.get("command_subscribed_groups", "") == group_text
            and self.config.get("command_subscribed_users", "") == user_text
        ):
            return

        self.config["command_subscribed_groups"] = group_text
        self.config["command_subscribed_users"] = user_text
        save_config = getattr(self.config, "save_config", None)
        if callable(save_config):
            save_config()

    async def _ensure_single_scheduler(self):
        """保证同一进程中只有当前实例持有定时推送后台任务。"""
        global _ACTIVE_SCHEDULER_TASK, _ACTIVE_SCHEDULER_OWNER

        async with _SCHEDULER_GUARD:
            active_task = _ACTIVE_SCHEDULER_TASK
            if active_task and not active_task.done():
                if _ACTIVE_SCHEDULER_OWNER == self._instance_token:
                    self._task = active_task
                    return

                logger.warning("检测到旧的每日AI资讯推送任务，准备切换到当前实例")
                active_task.cancel()
                try:
                    await active_task
                except asyncio.CancelledError:
            ...
            if self._task and not self._task.done():
                _ACTIVE_SCHEDULER_TASK = self._task
                _ACTIVE_SCHEDULER_OWNER = self._instance_token
                return

            self._task = asyncio.create_task(
                self._schedule_loop(),
                name=f"daily-ai-news-schedule-{self._instance_token[:8]}",
            )
            _ACTIVE_SCHEDULER_TASK = self._task
            _ACTIVE_SCHEDULER_OWNER = self._instance_token
    ...
    async def terminate(self):
        ...
        async with _SCHEDULER_GUARD:
            if _ACTIVE_SCHEDULER_OWNER == self._instance_token:
                _ACTIVE_SCHEDULER_TASK = None
                _ACTIVE_SCHEDULER_OWNER = None
